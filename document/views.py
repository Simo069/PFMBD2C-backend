...
# @api_view(['POST'])
# @permission_classes([IsAuthenticated])
# def search_in_documents(request):
#     """
#     Recherche sémantique dans tous les documents PDF de l'utilisateur
#     """
#     try:
#         # Valider la requête
#         data = request.data
...
#         top_k = data.get('top_k', 10)  # Nombre de résultats par défaut
...
#         chunk_ids = vector_db.search(request.user.id, query_embedding, top_k=top_k)
        
#         # Récupérer les chunks correspondants avec leurs métadonnées
#         chunks = Chunk.objects.filter(
#             id__in=chunk_ids,
#             user=request.user
#         ).select_related('pdf')
        
#         # Ordonner selon la similarité (l'ordre de chunk_ids est déjà trié par similarité)
...
#                 'chunk_id': chunk.id,
#                 'text': chunk.chunk_text,
#                 'similarity_score': self._calculate_similarity_score(query_embedding, chunk),  # À implémenter
#                 'pdf_id': chunk.pdf.id,
#                 'pdf_name': chunk.pdf.original_filename,
#                 'page_number': chunk.page_number,
#                 'chunk_index': chunk.chunk_index,
#                 'context': self._get_context(chunk)  # Pour montrer le contexte autour
#             })
...
# def _calculate_similarity_score(self, query_embedding, chunk):
#     """
#     Calcule le score de similarité entre la requête et le chunk
#     """
#     # Récupérer l'embedding du chunk depuis la base vectorielle
#     # Note: Vous devrez peut-être stocker les embeddings dans le modèle Chunk
#     # ou les recalculer à la volée
#     from .services.embedding_service import EmbeddingService
#     embedding_service = EmbeddingService()
#     chunk_embedding = embedding_service.generate_embedding(chunk.chunk_text)
...
# def _get_context(self, chunk, context_size=2):
#     """
